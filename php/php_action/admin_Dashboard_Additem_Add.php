...
if (isset($_REQUEST['sumbitToAddItem'])) {
              ...
              // output data of each row
                while($row = $result->fetch_assoc()) {
                    ...
                    if($name == $AddItem_itemName){
                        $picHasBeenUploaded = 0;       
                        break;
                        header("Location: ../../admin_Dashboard_Additem.php?addMsg=It has similar name!");
                    }else{
    ...
    if ($picHasBeenUploaded == 1) {
        ...
        if ($insert_query > 0) {
            header("Location: ../../admin_Dashboard_Additem.php?addMsg=Add Item Successfully");
        } else {
            unlink("../../img/itemPhotos/" . $photo);
            header("Location: ../../admin_Dashboard_Additem.php?addMsg=Can't Add Item");
        }
    } else {
        unlink("../../img/itemPhotos/" . $photo);
        header("Location: ../../admin_Dashboard_Additem.php?addMsg=Can't Add Item, It has similar name!");
    }
}
